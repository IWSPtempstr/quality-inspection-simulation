...
| S5 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Added a fixture-only `scheduler.evaluation` harness that reuses immutable snapshots plus the existing CP-SAT and fallback implementations to validate controlled hard rules, frozen-step invariance, fallback determinism, feasible-first execution, and equal-SLA change minimization. The reproducible report at `docs/product/scheduling-evaluation.md` documents the fixture inventory, commands, validation scope, and explicit non-goals; no API, callback, historical replay, or solver behavior was added. |
| A1 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Created the `services/ai-py` service skeleton defined by 4.3, including typed settings, prompt resources and loader, bounded internal FastAPI endpoints, request context/log redaction helpers, optional LLM JSON normalization, and strict request/response plus citation validation. A1 returns only contract-safe degraded placeholders and does not implement retrieval, memory, diagnosis reasoning, case indexing, or business writes. `contracts/openapi/ai-internal.yaml` now explicitly declares the implemented 401/403 and degraded response shapes alongside the existing bounded endpoint schemas. |
| A2 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Added the A2 retrieval foundation for versioned Chroma collections and BM25 indexes, typed metadata filters, in-memory activation/version repositories, and service-layer active-version plus fallback search with explicit degraded results. Coverage now proves standard-version/access filtering, center-isolated resolved-case filtering, active-version rollback search, and lexical fallback when Chroma is unavailable. A2 does not yet fuse ranks, rerank, answer with citations, or invoke the diagnosis workflow; those remain in A3/A4. |
| A3 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Added A3 hybrid standards retrieval above the A2 foundations: reciprocal-rank fusion over Chroma and BM25 results, local Cross-Encoder reranking with explicit degraded fallback, cited standard Q&A assembly, and deterministic impact-analysis output over the same cited set. The bounded AI knowledge path now returns grounded citations when evidence exists and degraded no-evidence when the retrieval backends or reranker are unavailable. A3 does not implement diagnosis-agent planning, case memory, review indexing, or new transport contracts; those remain in A4-A6. |
| A4 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Implemented the evidence-bounded `ExceptionDiagnosisAgent` on the existing internal diagnosis route with only six read-only tools, a strict five-call ceiling, and no write capability. Diagnosis now returns grounded results when cited standards/cases exist and returns structured evidence-insufficient or degraded results when required snapshots or retrieval evidence are unavailable. Coverage proves allowed tool sequencing, five-call ceiling, grounded diagnosis, evidence-insufficient behavior, degraded retrieval handling, and the `httpx.ASGITransport` internal API path. |
| A5 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Added Redis-bounded short diagnosis memory with explicit center/user/event/session scoping, 24-hour recent-turn retention, seven-day structured summary retention, and deterministic compression once recent turns exceed eight turns or the configured token budget. The diagnosis assistance path now records paired user/assistant turns only when a `session_id` exists, compresses older turns into structured user/assistant points, and never writes long-term cases or PostgreSQL review data. Coverage proves scope isolation, TTL expiry, turn-count compression, token-budget compression, and the session-aware diagnosis-service integration. |
| A6 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Added an explicit long-memory review/indexing boundary inside `services/ai-py`: reviewed exception cases are represented as PostgreSQL-owned review records plus Outbox-intended indexing records, and only approved cases are published into the current Chroma/BM25 resolved-case indexes. Coverage proves candidate creation stays non-persistent until review, approval emits index work, revoke removes indexed entries, and no unresolved case becomes retrievable. |
| A7 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Added a bounded AI evaluation gate covering approved-only resolved-case retrieval, center isolation, citation-presence invariants, and outage-safe degradation. Retrieval continues to require review-state, center/access-scope, and retention filters before evidence can be returned, and the evaluation checks now make those guarantees executable in tests. |
| A8 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Replaced the remaining A1 placeholders with bounded, schema-safe assistance services for persisted schedule-result explanation, deterministic failed-rule explanation, non-persistent case-candidate extraction, structured audit-filter suggestion, and notification-body drafting. The diagnosis route now also reports structured memory status from A5 while remaining read-only and center-scoped. No endpoint gained arbitrary tool dispatch, direct persistence, or mutation capability. |
| I1 | done | `docker compose -f deploy/compose/compose.yaml config`; `bash -n deploy/compose/scripts/backup-chroma.sh`; `bash -n deploy/compose/scripts/restore-chroma.sh`; `git diff --check` | Added a bounded deployment skeleton under `deploy/**`: Docker Compose for edge/API/worker/AI/scheduler plus PostgreSQL, RabbitMQ 4, Redis 7, Chroma, and operational stubs; Nginx reverse proxy with stdout/stderr structured logs; Chroma persistent volume and heartbeat health check; manual backup/restore helper scripts; environment example files for runtime configuration; and optional Prometheus + blackbox observability wiring. The stack keeps secrets out of tracked files, leaves service code untouched, and treats OIDC/partner/notification reachability as controlled deployment dependencies. |
| I2 | done | `git diff --check`; manual review against `DEV_SPEC.md` I2, sections 1, 5.2, 5.4, 6.1, 6.4, and Phase 5 boundaries | Added the one-time desensitized import document pack under `docs/migration/**`: overall runbook, manifest template, field-boundary and sanitization rules, reconciliation checklist, rollback runbook, and evidence template. The runbook keeps PostgreSQL as authority, treats Chroma/BM25 as rebuildable retrieval layers, forbids automatic promotion of historic incidents into approved long-term memory, and records explicit rollback triggers and evidence requirements. |
| I3 | done | `I3_WORKER_MODE=host bash tests/e2e/i3_rabbitmq_and_writeback.sh`; `bash -n tests/e2e/i3_rabbitmq_and_writeback.sh`; `docker compose -f deploy/compose/compose.yaml -f deploy/compose/compose.e2e.yaml --profile e2e config`; `git diff --check -- deploy tests/e2e spec.md` | Added a live I3 E2E harness under `tests/e2e/**` plus a compose override for partner-event and partner write-back validation. This turn fixed the runtime harness to always enable the `e2e` profile, kept bounded retries around `api-worker` startup, and completed a full live pass in the controlled environment through the script's host-worker mode after the Docker-built worker path proved bottlenecked by base-image pulls. The passing run exercised real PostgreSQL, RabbitMQ, Redis, Goose SQL migrations, resource-event consumption, rebuild-intent publication, approved-pending-writeback processing, formal schedule expansion, and recorded partner HTTP `PUT` verification end to end. |
| I4 | done | `GOROOT=/usr/local/go PATH=/usr/local/go/bin:$PATH GOCACHE=/tmp/go-build-cache /usr/local/go/bin/go test ./internal/clients/rabbitmq -run 'TestClient(PingRecoversAfterBrokerRestart|DeclaresDurableTopologyAndRoutesRetriesAndDLQ|PublishesInternalEventsWithBrokerConfirmation)$' -count=1`; `GOROOT=/usr/local/go PATH=/usr/local/go/bin:$PATH GOCACHE=/tmp/go-build-cache /usr/local/go/bin/go test ./internal/api -run 'Test(InjectedHealthProbesAggregateWithoutLeakingFailures|InjectedPostgresFailureMakesHealthUnavailable)$' -count=1`; `bash tests/e2e/i4_health_and_recovery.sh`; `bash tests/e2e/i4_scheduler_guardrails.sh`; `bash -n tests/e2e/i4_health_and_recovery.sh`; `python3 -m py_compile tests/e2e/oidc-stub/server.py tests/e2e/oidc-stub/mint_session.py tests/e2e/partner-recorder/server.py`; `docker compose -f deploy/compose/compose.yaml -f deploy/compose/compose.e2e.yaml --profile e2e --profile ops config`; `git diff --check -- services/api-go/internal/clients/rabbitmq/client.go services/api-go/internal/clients/rabbitmq/client_integration_test.go tests/e2e docs/migration spec.md` | I4 now has Docker-backed acceptance evidence for Chroma backup/restore smoke, sanitized health aggregation, unauthenticated session rejection, RabbitMQ/Redis/partner/PostgreSQL outage drills, a real public preview-approval race using a signed OIDC session cookie, an authenticated schedule-step start race, and Python-side scheduler guardrail coverage inside `tests/e2e/**`. The latest July 17, 2026 live pass proved one concurrent `approve` returns `200`, the loser returns `409`, only one `schedule.writeback` outbox row is inserted, one concurrent `schedule-steps/{step_id}/start` returns `200`, the loser returns `409`, the persisted notification set is deduplicated to the order creator plus same-center schedulers, and RabbitMQ health now returns to `available` after broker recovery without restarting the host-run `api-go` process. The targeted RabbitMQ client integration test also proves `Ping()` recovers after a real broker app stop/start and re-declares topology on reconnect. Bounded timing evidence for the authenticated flows (`health_baseline=29ms`, `preview_create=44ms`, `candidate_callback=25ms`, `approve_race=35ms`, `step_start_race=29ms`) is recorded as evidence only, not as a contractual target. |
| I5 | done | `git diff --check -- DEV_SPEC.md spec.md`; `git diff --name-status` | Added the approved narrow-scope I5 amendment to `DEV_SPEC.md` and executed the exact legacy runtime deletion scope only: `app.py`, `requirements.txt`, `api/**`, `web/**`, `agents/**`, `mcp_server/**`, `rag/__init__.py`, `rag/retriever.py`, `rag/vector_store.py`, `rag/index/index.faiss`, `rag/index/metadata.json`, `services/__init__.py`, `services/cp_sat_schedule_service.py`, `services/llm_client.py`, `services/mcp_client.py`, `services/notification_service.py`, `services/queue_service.py`, `services/scheduler_service.py`, `services/security_service.py`, `services/simulation_service.py`, and `services/tool_client.py`. Deferred review assets and current production runtime assets were left intact. |
| I6 | done | `git diff --check -- docs/audits/phase5-i5-legacy-runtime-candidates.md docs/audits/phase5-i5-dev-spec-amendment-proposal.md docs/audits/phase5-i6-precleanup-audit.md docs/audits/phase5-i6-final-cleanup-audit.md spec.md`; cache/temp path absence review | Completed the final cleanup audit in `docs/audits/phase5-i6-final-cleanup-audit.md`. The approved automatic-cleanup scope (`.ruff_cache/**`, `.mypy_cache/**`, `.pytest_cache/**`, `**/__pycache__`, `data/_validation_tmp/**`, `data/_synthetic_validation_tmp/**`, `data/_synthetic_large_validation_tmp/**`, and `data/_optimized_scheduling_eval_tmp/**`) had no remaining matches in the current worktree, so I6 performed no additional deletions. Remaining assets such as `apps/web/node_modules/**`, `services/api-go/node_modules/**`, `scripts/**`, deferred-review legacy code, and `data/simulation.db` are recorded as review-only and were not deleted. |
| I7 | in_progress | OpenAPI Generator validation; Spectral (0 errors, 89 existing quality warnings); Go `go test -count=1 ./...` including PostgreSQL/RabbitMQ/Redis Testcontainers; `go vet ./...`; fixed `golangci-lint v2.12.2` (0 issues); frontend lint/typecheck/86 Vitest tests/build; main Playwright 5/5 and demo 1/1; Compose render and shell validation | Opaque Redis BFF sessions, PKCE/nonce callback verification, CSRF enforcement, safe login UX, production static frontend/TLS Compose topology, secret-file configuration, credentialed partner/webhook delivery, and a real PostgreSQL shift TIME route regression are implemented. I7 remains in progress until operations supplies DNS/ACME contact, real OIDC issuer and redirect registration, mapped center/role claims, real partner and notification endpoints/credentials, root-owned secret files, firewall, backup destination, and rollback owner; live production TLS/OIDC/deployment drill is therefore not yet complete. |
| I8 | in_progress | Frontend typecheck/lint; 19 public-showcase Vitest checks; Pages-base-path showcase build; static bundle scan | Added an isolated public showcase build with in-bundle read-only display data, HashRouter workbench routes, no browser network/storage writes, unavailable business commands, and a least-privilege GitHub Pages workflow. The implementation is locally validated but cannot be deployed until the repository owner enables GitHub Actions and selects GitHub Actions as the Pages source. |
...
## Phase 3 S5 Cleanup Audit

| Path | Item | Purpose | Recommendation | Rationale |
| --- | --- | --- | --- | --- |
| `services/scheduler-py/src/scheduler/evaluation/**` | controlled-fixture evaluation harness | Measures offline correctness, frozen invariance, fallback determinism, and change-count comparisons without replaying APIs or historical datasets | keep | Formal S5 delivery boundary required by DEV_SPEC. |
| `services/scheduler-py/tests/evaluation/test_harness.py` | S5 regression suite | Proves the required fixture-only acceptance checks across CP-SAT and deterministic fallback outputs | keep | Required delivery evidence. |
| `docs/product/scheduling-evaluation.md` | reproducible evaluation report | Documents commands, fixtures, validation scope, and non-goals for S5 | keep | Required user-facing evidence for the completed scheduler evaluation task. |
| `services/scheduler-py/**/__pycache__` and tool caches | validation outputs | Local interpreter and analysis caches | delete | Ignored generated artifacts recreated by Ruff, mypy, and pytest. |

## Phase 4 A1-A8 Cleanup Audit

| Path | Item | Purpose | Recommendation | Rationale |
| --- | --- | --- | --- | --- |
| `services/ai-py/src/ai_service/services/cases.py` | long-memory review/index worker boundary | Keeps approved-case review, outbox publication, and index mutation out of the diagnosis flow | keep | Formal A6 production boundary, not a throwaway helper. |
| `services/ai-py/src/ai_service/services/evaluation.py` | retrieval/diagnosis gate checks | Makes approval, isolation, citation, and outage guarantees executable | keep | Required A7 verification artifact. |
| `services/ai-py/tests/test_cases.py` | A6-A8 regression suite | Verifies review/index lifecycle, retrieval gate behavior, and bounded candidate extraction | keep | Required delivery evidence. |
| `services/ai-py/**/__pycache__` and tool caches | validation outputs | Local interpreter and analysis caches | delete | Ignored generated artifacts recreated by Ruff, mypy, and pytest. |
