...
| A4 | done | `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning ruff check .`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning mypy src`; `PYTHONPATH=src /root/anaconda3/bin/conda run --no-capture-output -n agent-learning pytest -q` | Implemented the evidence-bounded `ExceptionDiagnosisAgent` on the existing internal diagnosis route with only six read-only tools, a strict five-call ceiling, and no write capability. Diagnosis now returns grounded results when cited standards/cases exist, returns a structured 200 evidence-insufficient result when required snapshots are absent, and returns a structured 503 degraded result only when retrieval dependencies fail without usable evidence. Coverage proves allowed tool sequencing, five-call ceiling, grounded diagnosis, evidence-insufficient behavior, degraded retrieval handling, and the `httpx.ASGITransport` internal API path. |
...
| A6-A8 | pending | AI checks and G8/A8 interface mapping defined in DEV_SPEC | A5 completed Redis short memory and structured session compression. A6 is next for the PostgreSQL review queue, Outbox, and approved exception-memory Chroma indexing. |
